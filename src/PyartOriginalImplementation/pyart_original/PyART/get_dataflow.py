...
def get_ngram_score_kenlm(apis,current_dataflow,baseflag,basetype,callee,thread=""):
	flows=[]
	ngram_scores={}
	...
	if baseflag==1:
		...
		for flow in current_dataflow:
			for api in apis:
				...
				ngram_scores[api]=total_logprob
    
	#print(flows,apis,callee)
	# dataflow_ngram_scores=get_ngram_scores(flows,apis,callee,thread)
	#print('data1',dataflow_ngram_scores)
	return ngram_scores

def get_ngram_scores(flows,apis,callee,thread=""):
	s=''
	#print(apis)
	#print(flows)
	ngramscore={}
	for flow in flows:
		s=s+flow+'\n'
	with open('../Ngram-output/test_' + thread + '.txt','w+') as f:
		f.write(s)
	#print(s)
	#os.chdir('dataflow/')

	path_to_srilm = "../utils//Linux/srilm-1.7.3/lm/bin/i686-m64/ngram"
	if (config.get("System", "os").upper() == "MACOS"):
		path_to_srilm = "../utils//MacOS/srilm-1.7.3/lm/bin/macosx/ngram"

	os.system(path_to_srilm +  ' -ppl ../Ngram-output/test_' + thread + '.txt -order 4 -lm ../trainfile.lm -debug 2 > ../Ngram-output/'+ callee + '_' + thread + '.ppl')
	with open('../Ngram-output/'+ callee + '_' + thread + '.ppl',encoding='ISO-8859-1') as f:
		lines=f.readlines()
	...
	#print(ngramscore)
	os.system('rm ../Ngram-output/'+callee+'_' + thread+'.ppl')
	#os.chdir('../')
	return ngramscore
	
def get_ngram_score(apis,current_dataflow,baseflag,basetype,callee,thread=""):
	flows=[]
	if baseflag==1:
		for api in apis:
			if api.startswith('__') or re.match('[A-Z0-9_]+$',api) or api.strip()=='_':
...
def get_dataflow_scores(apis,maxflow,current_dataflow,ft,callee,thread=""):
	dataflow_ngram_score={}
	basetypes=['int','str','float','list','dict','set','tuple','buffer','frozenset','complex','bool','unicode','bytes','bytearray']
	basetype=''
	baseflag=0
